    ...
    if (op in [0,7,8]) and ind % 2 == 1:
      while (count > 0):
        for base in MDfields[ind]:
          res.append((posRef,posRead,seq[posRead]))
          posRef += 1
          posRead += 1
          count-= 1
...
reflength = samfile.get_reference_length(samfile.get_reference_name(0)) - 37
...
#		NM = read.get_tag("NM") 	
		...
		a = parseMDwithCigar(MD,cigar,read.seq)
		a=list(filter(lambda x:x[0]<=reflength,a)) 
#		a=list(filter(lambda x:x[0]>27,a))
		NM=len(a)
		print("%d\t%d\t%s\t%s\t%s"%(read.pos,len(read.seq),UMI,NM,a))
